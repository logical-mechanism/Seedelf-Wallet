...
///
/// This is the real entry point; `main.rs` is a thin shim that awaits it. It
/// lives in the library target so integration tests can link against the
/// command modules — see `tests/cli/`.
pub async fn run() {
