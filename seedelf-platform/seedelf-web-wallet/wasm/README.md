...
| `draftWithdraw(key, requestJson)` | A withdrawal, step 1 (`build::sweep` or `sweep_all`). The request carries `network`, the `epoch_params` row, the spendable contract UTxOs (checked as for a mint), `to` (a bech32 key address on this network), `lovelace` (`null` for Max) and `tokens`. Max takes the 20 largest UTxOs. Returns `{ seed, draftCbor, inputs }`. |
| `finishWithdraw(key, requestJson)` | Step 2: plus `seed` and `evaluation`. Returns the unsigned transaction and `{ to, max, lovelace, tokens, fee, changeLovelace, changeTokens, changeOutputs, inputs, left }`: what the address receives, what comes back, and how many UTxOs Max left. |
...
| `checkWithdrawAddress(address, network)` | Throws the reason a withdrawal can't pay an address: not an address, a script, a stake address, the other network. |
