...
| Home | Unlocked | Two tabs. **Private:** the private balance with round **Receive** (your Seedelfs' names), **Send** (to Seedelfs), **Make public** and **Create** (a Seedelf); the first five tokens and **View all**. **Public:** the public account with **Receive**, **Send** and **Make private**, and tokens. Until the first reading arrives, a splash covers it. A new wallet gets *Get started* (fund, create, make private). Refresh sits under the tabs, and a sent transaction shows as a banner until it confirms. The eye beside each balance hides the amounts; on mainnet, each balance's value in the chosen currency sits under it. An ADA Handle in the private balance gets a warning. The lock button is in the top bar. |
...
| Settings | The gear in the top bar | **Contacts** (add, edit, delete; encrypted on the device), **Collateral** (see it, set it, reclaim it), where the wallet opens (a full tab or the side panel), ADA's value in a currency (mainnet), spending the staking rewards, **Lock after**, **Show recovery phrase** (the password again first), **Check recovery phrase** (a written copy: yes or no), **Change password**, **Remove wallet** (typed confirmation), and About: the version, the network, links to the source and the privacy policy |
...
| Make public | From the Private tab | Up to 20 addresses or `$handle`s, read as they're typed, with a warning if one is your own public account. An ADA amount and optional tokens (the minimum worked out as for a move-in), or Max (up to 20 UTxOs, every token). Then a review of where it goes, what's sent, the fee and the change, then Send, when giveme.my is asked for the collateral. |
