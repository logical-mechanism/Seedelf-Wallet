...
| Home | Unlocked | Two tabs. **Private:** the private balance with round **Receive** (your Seedelfs' names), **Send** (to Seedelfs), **Make public** and **Create** (a Seedelf); the first five tokens and **View all**. **Public:** the public account with **Receive**, **Send** and **Make private**, and tokens. Until the first reading arrives, a splash covers it. A new wallet gets *Get started* (fund, create, make private). Refresh sits under the tabs, and a sent transaction shows as a banner until it confirms. The lock button is in the top bar. |
...
| Activity | The row near the bottom of each Home tab | Newest first, grouped by day; an entry opens its details and Cardanoscan. **Private:** from the device, encrypted, no requests. **Public:** 20 at a time from Koios, with Load more. **Refresh** reads again without going back to Home. |
...
| Receive | From the Public tab | The receive address as a QR code and text, with copy, and the stake address |
