...
| Home | Unlocked | Two tabs. **Seedelf:** the balance with round **Receive** (your seedelfs' names), **Send** (to a seedelf), **Withdraw** and **Create** (a seedelf); the first five tokens and **View all**; your seedelfs with Copy and Remove on each. **Cardano account:** the balance with **Receive**, **Send** and **Move in**, and tokens. Until the first reading arrives, a splash covers it. A new wallet gets *Get started* (fund, create, move in). Refresh sits under the tabs, and a sent transaction shows as a banner until it confirms. The lock button is in the top bar. |
...
| Settings | The gear in the top bar | **Contacts** (add, edit, delete; encrypted on the device), **Show recovery phrase** (the password again first), **Change password**, **Remove wallet** (typed confirmation), and About: the version, the network, links to the source and the privacy policy |
...
| Receive (Seedelf) | From the Seedelf tab | Each of your seedelfs by its tag, with its whole name and Copy; with none yet, a way to create one |
