...
| Home | Unlocked | The Seedelf balance (ADA, tokens, **Send to a seedelf**, your seedelfs, **Create a seedelf**), the Cardano account (ADA, tokens, receive address with copy and QR, stake address, **Move in**), the Seedelf identity, and Refresh. A sent move-in, mint or transfer shows as a banner until it confirms. The lock button is in the top bar. |
