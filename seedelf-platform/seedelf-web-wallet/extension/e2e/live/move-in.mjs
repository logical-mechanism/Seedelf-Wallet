...
// preprod Koios, with nothing intercepted, and move funds from the test
// wallet's Cardano account into its Seedelf balance. It submits a REAL
// preprod transaction.
//
//   npm run build && node e2e/live/move-in.mjs [ada]      (default 10 ADA)
//
// Reads the test wallet's phrase from .preprod-test-wallet.txt (gitignored).
// Picks every token the account holds, so the token path runs too.
...
try {
  ...
  log("before:", "seedelf", await page.getByTestId("seedelf-lovelace").textContent(), "| cardano", await page.getByTestId("cardano-lovelace").textContent());
  ...
  await page.screenshot({ path: "test-results/live-move-in-review.png", fullPage: true });
  ...
  await banner.filter({ hasText: "Move-in confirmed" }).waitFor({ timeout: 10 * 60_000 });
  log("confirmed");
  await page.getByTestId("updated").filter({ hasText: "Updated" }).waitFor({ timeout: 60_000 });
  await page.waitForTimeout(1500);
  log("after:", "seedelf", await page.getByTestId("seedelf-lovelace").textContent(), "| cardano", await page.getByTestId("cardano-lovelace").textContent());
  const tokens = await page.getByTestId("seedelf-tokens").innerText().catch(() => "(none)");
  log("seedelf tokens:", tokens.replace(/\n/g, " | "));
  await page.screenshot({ path: "test-results/live-move-in-after.png", fullPage: true });
