...
- Move in, Send and Withdraw list every token of the balance in their forms. With hundreds, they need a token picker with search (Lace's "add assets" sheet). They should also name tokens as the lists do (`src/ui/tokens.ts`), with tickers and logos.
