...
| 5 | Vault and lock | 🚧 | SecretBox vault, create/restore onboarding (restore has per-word BIP39 autocomplete, like Lace and Eternl), unlock, `chrome.storage.session`, auto-lock, unlock back-off. **Full plan: [plans/chunk-05-vault-and-lock.md](plans/chunk-05-vault-and-lock.md).** |
...
- **2026-09-23: chunk 5 planned; not started.**
  - The branch `web-wallet/vault-lock` exists, and its full plan is [plans/chunk-05-vault-and-lock.md](plans/chunk-05-vault-and-lock.md). A new session starts there.
  - Two user requests are folded in:
    - Per-word phrase autocomplete, in chunk 5.
    - A much closer match to Lace's dark mode, in chunk 11, or earlier if the user wants it.
