...
- **2026-09-24: chunk 12 done** (`web-wallet/style-flow`). Plan: [plans/chunk-12-style-flow.md](plans/chunk-12-style-flow.md).
  - **What landed:** the user's 25 findings, each with its decision in the plan's list. Items 21–25 came after the PR opened: UTxOs on each Home tab, locks on both sides (from each row too), the collateral in Settings, and Refresh on UTxOs and Activity. The store images are regenerated (Home changed); the listing text isn't, until chunk 13's new positioning.
