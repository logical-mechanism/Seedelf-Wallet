...
| 12 | Style and flow | ✅ | The user tested the built wallet and sent findings; 23 items, each decided with the user. Among them: a loading splash, a Tokens screen and a bundled token list, a token picker with any amount of each, Settings, Activity, Contacts, an incremental contract scan, Send from the Cardano account, the minimum ADA worked out, Receive on the Seedelf tab, spending everything under the account's payment keys, a UTxOs screen on both sides with locks, and the Cardano account's collateral in Settings. **Plan: [plans/chunk-12-style-flow.md](plans/chunk-12-style-flow.md).** |
...
- **2026-09-24: chunk 12 done** (`web-wallet/style-flow`). Plan: [plans/chunk-12-style-flow.md](plans/chunk-12-style-flow.md).
  - **What landed:** the user's 23 findings, each with its decision in the plan's list. Items 21–23 came after the PR opened: UTxOs on each Home tab, locks on both sides, and the collateral in Settings. The store images are regenerated (Home changed); the listing text isn't, until chunk 13's new positioning.
