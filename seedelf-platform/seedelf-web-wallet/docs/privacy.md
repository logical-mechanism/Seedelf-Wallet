...
- **Network:** Koios and giveme.my see the user's IP address, and Koios has no Tor access. A VPN helps; see the root README's IP-tracking section.
  ...
  - **Reviewing a private spend tells Koios nothing new** (since the crypto review, 2026-09-25): its scripts are measured in the wallet. Before, a draft went to Koios's Ogmios, and its proofs said which contract UTxOs were yours, even for a review never sent. Only Send submits it.
...
- **Lovejoin** (chunk 16, preprod only): a mixer of 10 ₳ boxes. A box goes in from one place and comes out, after other people's mixes, as one of many boxes.
  ...
  - **Timing and amounts:** each box waits a random while (1 to 6 hours by default, drawn from the browser's secure random source since the crypto review) and comes back at the first unlock after it. Boxes due together come back one at a time: one goes, and the others each wait a fresh 5 to 60 minutes, so a wallet locked for hours never sends them in one burst. Bringing one back early, or all at once, makes them easier to match. The boxes are all 10 ₳, so their amounts say nothing.
