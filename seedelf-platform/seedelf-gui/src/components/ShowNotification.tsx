...
function ariaForVariant(v: NotificationVariant) {
    return v === "error" ? { role: "alert", ariaLive: "assertive" as const }
...
export function ShowNotification({
    ...
    const [localVariant, setLocalVariant] = useState<NotificationVariant>(variant);
    ...
    return (
        <div
            className={`fixed bottom-16 right-4 z-50 rounded shadow-lg px-4 py-3 flex items-center gap-2 transition-transform duration-150 ${variantClasses}`}
            role={role}
            aria-live={ariaLive}
        >
            <span>{localMsg}</span>
