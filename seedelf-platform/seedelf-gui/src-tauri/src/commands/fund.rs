...
#[tauri::command(async)]
pub async fn fund_seedelf(network_flag: bool, addr: String, seedelf: String) -> String {
    ...
    let output = match session::with_key(|sk| build_fund_seedelf(config, network_flag, addr, seedelf, *sk))
    {
        Ok(v) => v.await,
        _ => return String::new(),
    };
