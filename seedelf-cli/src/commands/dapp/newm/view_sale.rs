...
use crate::commands::dapp::newm::constants::{MAINNET_POINTER_POLICY_ID, PREPROD_POINTER_POLICY_ID, USE_USD_FLAG};
...
pub async fn run(args: ViewSaleArgs, network_flag: bool) -> Result<(), String> {
    ...
    match nft_utxo(policy_id, token_name, network_flag).await {
        Ok(utxos) => {
            ...
            println!("{} {}", "\nTokens Left On Sale:".bright_magenta(), tokens.quantity_of(bundle.pid, bundle.tkn).unwrap());
            if cost.pid == USE_USD_FLAG {
                // Convert to f64 for floating-point division
                let result = cost.amt as f64 / 1_000_000.0 / 1_000_000.0;
                println!("{} {} {}", "Each Stream Token is".yellow(), result, "USD".yellow())
            } else {
                println!("{} {} {}", "Each Stream Token is".yellow(), format!("{:.6}", cost.amt as f64 / 1_000_000.0), "NEWM".yellow())
