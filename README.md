...
Direct tracking methods are not feasible as long as the privacy-preserving techniques are followed, as noted above. However, implicit tracking methods (ITMs) do exist even under the assumption of privacy preservation.

The first ITM is entering the wallet for the first time as a CIP30 wallet will have to be used to mint the original seedelf. This requirement means that an outside wallet, $\omega$, is now linked to a seedelf, $\sigma$. The receivability and spendability of $\sigma$ remain hidden, but this linking will taint $\omega$ as a wallet that has interacted with Seedelf and is attempting to be stealthy. The user may use the `util mint` command to do a stealth seedelf mint producing $\sigma^{'}$, dropping all linkability with $\omega$, after the funding of $\sigma$. The original $\sigma$ may be removed after $\sigma^{'}$ minting.
